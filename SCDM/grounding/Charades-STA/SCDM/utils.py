...
def conv2d(x, input_filters, output_filters, kernel_h,kernel_w, strides, name='conv'):
    ...
    with tf.variable_scope(name) as scope:
        ...
        bias = tf.constant(0.0, shape=[output_filters])
        bias = tf.get_variable('bias', initializer=bias)
        # 对于输入x,其中in_height=1, in_width=64, in_channels=512， 所以卷积的对象是时间长度，而不是特征，此处特征被当作通道
        x_conv = tf.nn.conv2d(x, weight, strides=[1, strides, strides, 1], padding='SAME', name='conv') # padding=SAME,因此卷积后时间维度不会变化
        return tf.nn.bias_add(x_conv,bias)



def scdm_norm(x, sentence_fts):
    """
    scdm normalization，使用conditional factor(sentence_fts)求出modulated feature unit
    """
    with tf.variable_scope('scdm_norm',reuse=tf.AUTO_REUSE) as scope:
        # 求当前layer的平均值和方差
        mean, var = tf.nn.moments(x, axes=[1, 2], keep_dims=True)
        dim = x.shape[-1]
        # sentence_fts变形来作为dense输入
        sentence_fts_shape = np.shape(sentence_fts)
        sentence_fts_reshape = tf.reshape(sentence_fts,[-1,sentence_fts_shape[-1]])
        # 用两个全连接层来求出beta和gamma
        beta = tf.layers.dense(sentence_fts_reshape,dim,activation=tf.tanh,use_bias=True,name='sentence_beta')
        gamma = tf.layers.dense(sentence_fts_reshape,dim,activation=tf.tanh,use_bias=True,name='sentence_gamma')
        beta = tf.reshape(beta,[sentence_fts_shape[0],sentence_fts_shape[1],dim])
        gamma = tf.reshape(gamma,[sentence_fts_shape[0],sentence_fts_shape[1],dim])
        beta = tf.expand_dims(beta,1)
        gamma = tf.expand_dims(gamma,1)
        # beta和gamma作为批正则化参数求出modulated feature unit
        x = tf.nn.batch_normalization(x, mean, var, beta, gamma, 1e-6)
        return x


def scdm_relu_conv(image,sentence_fts,is_training,input_filters, output_filters, kernel_h,kernel_w, strides,name='in_relu_conv1'):
    """
    SCMD处理+Relu激活+时序卷积
    """
    with tf.variable_scope(name) as scope:
        image_in = scdm_norm(image, sentence_fts)
        image_relu = tf.nn.relu(image_in)
...
def bidirectional_GRU(options, inputs, inputs_len, cell = None, cell_fn = tf.contrib.rnn.GRUCell, units = 256, layers = 1, scope = "Bidirectional_GRU", output = 0, is_training = True, reuse = None):
    ...
    Args:
        inputs:     rnn input of shape (batch_size, timestep, dim)   # timestep = options[max_sen_len]
    ...
    with tf.variable_scope(scope, reuse = reuse):
        ...
        # 是output而不是outputs,作为一个输出选项且在此模型中值为0,返回前向输出tensor和后向输出tensor的拼接,维度为(batch_size,max_sen_len,256*2=512)
        if output == 0:
...
def avg_sentence_pooling(options, memory, memory_len = None, scope = "avg_sentence_pooling"):
    """
    返回global sentence representation,通过对word进行一个简单的averaging, memory维度(batch_size,max_sen_len,512)
    """

    with tf.variable_scope(scope):
        # 根据各batch中的sentence长度生成掩码
        avg_attn = tf.cast(tf.cast(tf.sequence_mask(memory_len, maxlen=options['max_sen_len']),tf.int32),tf.float32)
        # 将掩码变成平均系数,如一个长度为2的句子,经处理后的平均系数变成[1/2,1/2,0,...,0]
    ...
    return tf.reduce_sum(attn * memory, 1) # 将一个sentence中的所有word的feature进行平均,维度为(batch_size,512)
...
def attention(inputs, units, weights, scope = "attention", memory_len = None, reuse = None):
    ...
    with tf.variable_scope(scope, reuse = reuse):
        outputs_ = []
        weights, v = weights
        a = list(zip(inputs,weights))
        # 通过zip,分别计算word_sequence(15,512)和w_v(512,256)、unit_video_fts(512,)和w_k(512,256)的矩阵和，经过变形后进行加和
