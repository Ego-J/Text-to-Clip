...
def scdm_relu_conv(image,sentence_fts,is_training,input_filters, output_filters, kernel_h,kernel_w, strides,nameid):
    with tf.variable_scope('scdm_relu_conv'+str(nameid)) as scope:
...
def scm_norm(x, sentence_fts):
    with tf.variable_scope('scm_norm',reuse=tf.AUTO_REUSE) as scope:
        mean, var = tf.nn.moments(x, axes=[1, 2], keep_dims=True)
        dim = x.shape[-1]
        beta = tf.layers.dense(sentence_fts,dim,activation=tf.tanh,use_bias=True,name='sentence_beta')
        gamma = tf.layers.dense(sentence_fts,dim,activation=tf.tanh,use_bias=True,name='sentence_gamma')
        beta = tf.tile(tf.expand_dims(tf.expand_dims(beta,1),2),[1,1,1,1])
        gamma = tf.tile(tf.expand_dims(tf.expand_dims(gamma,1),2),[1,1,1,1])
        x = tf.nn.batch_normalization(x, mean, var, beta, gamma, 1e-6)
        return x


def scm_relu_conv(image,sentence_fts,is_training,input_filters, output_filters, kernel_h,kernel_w, strides,nameid):
    with tf.variable_scope('scm_relu_conv'+str(nameid)) as scope:
        image_in = scm_norm(image, sentence_fts)
        image_relu = tf.nn.relu(image_in)
        image_conv = conv2d(image_relu, input_filters, output_filters, kernel_h,kernel_w, strides)
    return image_conv 
