...
output_path = 'D:\\Data\\Text-to-Clip\\SCDM\\data\\TACOS\\h5py\\'
splitdataset_path = 'D:\\Data\\Text-to-Clip\\SCDM\\data\\TACOS\\datasplit_info\\tacos_split.npz'
train_captions_path = 'D:\\Data\\Text-to-Clip\\SCDM\\data\\TACOS\\datasplit_info\\train.json'
val_captions_path = 'D:\\Data\\Text-to-Clip\\SCDM\\data\\TACOS\\datasplit_info\\val.json'
test_captions_path = 'D:\\Data\\Text-to-Clip\\SCDM\\data\\TACOS\\datasplit_info\\test.json'
...
def driver(dataset, output_path):
    ...
    cnt = 0
    batch_id = 1
    List = np.load(splitdataset_path,encoding="latin1")[dataset] # get the train,val or test training video name
    print(List)
    for iii in range(len(List)):
        video_name = str(List[iii],encoding="ascii")
        for capidx, caption in enumerate(info[video_name]['sentences']):
            if len(caption.split(' ')) < 35:

                g_left,g_right = get_ground_truth_position(info[video_name]['timestamps'][capidx])
                if g_left == -1 or g_right == -1:
                    continue

                g_position = [g_left,g_right]
                anchor_input = generate_anchor_params(all_anchor_list,g_position)
                video_names_list.append(video_name.encode())
                video_duration_list.append(info[video_name]['duration'])
                sentence_list.append(unicodedata.normalize('NFKD', caption).encode('ascii','ignore'))
                ground_interval_list.append(info[video_name]['timestamps'][capidx])
...
def shuffle_train_data():
    ...
    video_data_path_train = output_path+'train\\train.txt'
